...
export function PdfCVUploader({ onExtracted, t }: Props) {
  ...
  async function handleFile(file: File) {
    if (file.type !== 'application/pdf') {
      setError(t.applications.pdfOnlyPDF);
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setError(t.applications.pdfTooLarge);
      return;
    }
    setError('');
    setLoading(true);
  ...
  return (
    <div
      className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-5
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div
          className="w-10 h-10 rounded-lg bg-sky-50 dark:bg-sky-400/10
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <p className="font-mono text-[10px] text-slate-400 mt-0.5">{t.applications.pdfUploaderDesc}</p>
        </div>
        <button
          type="button"

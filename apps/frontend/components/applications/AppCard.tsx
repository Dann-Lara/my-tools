...
export function AppCard({
  ...
  const statusMap: Record<AppStatus, { label: string; color: string }> = {
    pending: {
      label: ta.statusPending,
      color:
        'text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-400/30 bg-amber-50 dark:bg-amber-400/5',
    },
    in_process: {
      label: ta.statusInProcess,
      color: 'text-sky-700 dark:text-sky-400 border-sky-200 dark:border-sky-400/30 bg-sky-50 dark:bg-sky-400/5',
    },
    accepted: {
      label: ta.statusAccepted,
      color:
        'text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-400/30 bg-emerald-50 dark:bg-emerald-400/5',
    },
    rejected: {
      label: ta.statusRejected,
      color: 'text-red-700 dark:text-red-400 border-red-200 dark:border-red-400/30 bg-red-50 dark:bg-red-400/5',
    },
  };
  ...
  async function generateAnswers() {
    ...
    try {
      ...
      setQaError(e instanceof Error ? e.message : 'Error generando respuestas');
    } finally {
      setQaLoading(false);
    }
  }
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 flex-wrap">
              <div>
                <p className="font-mono text-[13px] font-semibold text-slate-800 dark:text-slate-200">
                  {app.position}
                </p>
                <p className="font-mono text-[11px] text-sky-600 dark:text-sky-400 mt-0.5">{app.company}</p>
              </div>
              <span
                className={`font-mono text-[9px] uppercase tracking-wider px-2 py-0.5 rounded border shrink-0 ${meta.color}`}
              >
                {meta.label}
              </span>
            </div>
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 p-4 space-y-3 bg-slate-50/50 dark:bg-slate-900/30">
          ...
          <pre className="font-mono text-[10px] text-slate-600 dark:text-slate-400 leading-relaxed
            ...
            {cvLang === 'es'
              ...
              : app.cvGeneratedEn ?? app.cvGeneratedEs ?? ''}
          </pre>

          <div className="flex gap-2 flex-wrap">
            <button
              onClick={() => printATS(app.cvGeneratedEs ?? '', 'es', app.position, app.company)}
            ...
            <button
              onClick={() => printATS(app.cvGeneratedEn ?? '', 'en', app.position, app.company)}
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 p-4 space-y-4 bg-violet-50/30 dark:bg-violet-900/5">
          ...
          <div>
            ...
            <textarea
              ...
              rows={5}
              placeholder={
                ta.interviewQuestionsPlaceholder ??
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              disabled={qaLoading || !questions.trim()}
              className="btn-primary text-[10px] py-2 px-4 flex items-center gap-2 disabled:opacity-40"
            >
              {qaLoading ? (
                <>
                  <Spinner sm />
                  {ta.generatingAnswers ?? 'Generando...'}
                </>
              ) : (
                <>
                  <IconSpark />
                  {ta.generateAnswers ?? 'Generar respuestas'}
                </>
              )}
            </button>
            ...
            <div className="space-y-2">
              ...
              >
                {qaSaving ? (
                  <>
                    <Spinner sm />
                    {ta.savingAnswers ?? 'Guardando...'}
                  </>
                ) : (
                  <>
                    <IconSave />
                    {ta.saveAnswers ?? 'Guardar preguntas y respuestas'}
                  </>
                )}
              </button>
            </div>

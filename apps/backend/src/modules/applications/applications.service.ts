...
import { esc, extractJson, withRetry, cleanCvText } from './applications.utils';
import type {
...
${app.cvGeneratedEn}`;
  ...
  async answerInterviewQuestions(
    ...
    const cvContext =
      ...
      '=== ADDITIONAL CONTEXT ===\n' +
      'SKILLS (full): ' +
      esc((baseCV.skills ?? '').slice(0, 500)) +
      '\n' +
      'LANGUAGES: ' +
      esc(baseCV.languages ?? '') +
      '\n' +
      'CERTS: ' +
      esc((baseCV.certifications ?? '').slice(0, 300));
  ...
  > {
    ...
    const truncated =
      dto.pdfText.length > 8000 ? dto.pdfText.slice(0, 8000) + '\n...[truncated]' : dto.pdfText;
    ...
    const systemMessage =
      ...
      ((dto as any).lang === 'en' ? 'English' : 'Spanish') +
      '.\n' +
  ...
  async evaluateBaseCV(dto: EvaluateCvDto): Promise<CvEvaluationResult> {
    ...
    if (pendingFields.includes('summary'))
      cvLines.push('SUMMARY: ' + esc((dto.summary ?? '').slice(0, 600)));
    if (pendingFields.includes('experience'))
      cvLines.push('EXPERIENCE: ' + esc((dto.experience ?? '').slice(0, 1200)));
    if (pendingFields.includes('education'))
      cvLines.push('EDUCATION: ' + esc((dto.education ?? '').slice(0, 400)));
    if (pendingFields.includes('skills'))
      cvLines.push('SKILLS: ' + esc((dto.skills ?? '').slice(0, 300)));
    if (pendingFields.includes('languages'))
      cvLines.push('LANGUAGES: ' + esc((dto.languages ?? '').slice(0, 200)));
    if (pendingFields.includes('certifications'))
      cvLines.push('CERTS: ' + esc((dto.certifications ?? '').slice(0, 300)));

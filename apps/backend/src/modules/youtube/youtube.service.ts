...
import {
  generateNichesWithAI,
  generateScriptWithAI,
  generateAIPromptsWithAI,
  generateContentIdeasWithAI,
} from './ai/youtube-ai';
import { CreateChannelDto } from './dto/create-channel.dto';
import {
  UpdateIdeaSeoDto,
  UpdateIdeaMetricsDto,
  UpdateIdeaStatusDto,
  GeneratePromptsDto,
...
export class YoutubeService {
  ...
  async generateAIPrompts(
    ...
    const prompts = await generateAIPromptsWithAI(idea.title, idea.script || '', promptType);

    const existingPrompts = await this.promptRepo.find({
      where: { ideaId },
    });
    const maxBatch =
      existingPrompts.length > 0 ? Math.max(...existingPrompts.map((p) => p.generationBatch)) : 0;
  ...
  async canAccessModule(userId: string, role: string, moduleName: string): Promise<boolean> {
    ...
    if (
      (!visibility.allowedRoles || visibility.allowedRoles.length === 0) &&
